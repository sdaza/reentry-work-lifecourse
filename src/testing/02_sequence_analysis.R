...
file.copy(paste0(path_paper, "output/tab_cluster_jobs.tex"),
...
create_plots(seq_data_job_crime_em_se, cl_job_crime_em_se_4[[1]],
    ...
    order = "sql")
file.copy(paste0(path_paper, "output/seq_job_crime_em_se_4_clusters.pdf"),
...
slabels = c("None", "Crime", "Self-employed", "Self-employed-Crime",
    "Employed", "Employed-Crime")
glabels = c("Unemployed", "Offenders", "Self-employed", "Employed")
...
trans = transMatGroup(seq_data_job_crime_em_se, groupv = cluster_vector, slabels = slabels, 
    states = 0:5, sample = 1000, glabels = glabels)
trans[, Transition := paste0("\\quad ", Transition)]
trans[, Transition := gsub("->", "$\\\\rightarrow$", Transition)]
...
addtorow = list()
addtorow$pos = as.list(c(6, 12, 18, 24, 30))
addtorow$command = as.vector(rep("\\addlinespace[12pt] \n", 5))
